...
def airport_performance_analysis(request):
    ...
    try:
        ...
        logger.info(f"Airport Performance Analysis Request - Airport: {airport_code}")
        ...
        SELECT 
            ...
            COUNT(CASE WHEN CANCELLED = 1.0 THEN 1 END) as cancelled_departures,
            COUNT(CASE WHEN DIVERTED = 1.0 THEN 1 END) as diverted_departures,
            ...
            COUNT(DISTINCT OP_CARRIER) as unique_airlines,
            COUNT(DISTINCT DEST) as unique_destinations
        FROM combined_data
        """
        
        if airport_code:
            base_query += f" WHERE ORIGIN = '{airport_code}'"
        ...
        for row in results:
            ...
            # Calculate complexity score based on volume and operations
            complexity_score = min((total_departures / 1000) * 10, 10)  # Scale to 0-10
        ...
        if airport_code:
            trend_query += f" AND ORIGIN = '{airport_code}'"
